...
d) 设置超时时间
e) 开启软硬件加速
f) 网页端的优化 由网页的前端工程师优化网页，或者说是和移动端一起，将网页实现增量更新，动态更新。app内置css,js文件并控制版本
...
a) 首先，在浏览器地址栏中输入url
b) 浏览器先查看浏览器缓存-系统缓存-路由器缓存，如果缓存中有，会直接在屏幕中显示页面内容。若没有，则跳到第三步操作。
c) 在发送http请求前，需要域名解析(DNS解析)，解析获取相应的IP地址。
d) 浏览器向服务器发起tcp连接，与浏览器建立tcp三次握手。
e) 握手成功后，浏览器向服务器发送http请求，请求数据包。
f) 服务器处理收到的请求，将数据返回至浏览器
g) 浏览器收到HTTP响应
h) 读取页面内容，浏览器渲染，解析html源码
i) 生成Dom树、解析css样式、js交互
j) 客户端和服务器交互
k) ajax查询
